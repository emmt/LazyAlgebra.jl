...

{
    ...
    "title": "Construction",
    "category": "section",
    "text": "A sparse operator can be built as follows:SparseOperator(I, J, C, rowdims, coldims)where I and J are row and column indices of the non-zero coefficients whose values are specified by C and with rowdims and coldims the dimensions of the rows and of the columns.  Appart from the fact that the rows and columns may be multi-dimensional, this is very similar to the [sparse][sparse] method in [SparseArrays][SparseArrays] standard Julia module.Another possibility is to build a sparse operator from an array or from a sparse matrix:S = SparseOperator(A)where A is an array (of an y sub-type of AbstractArray) or a sparse matrix (of type [SparseMatrixCSC][SparseMatrixCSC]).  If A is an array with more than 2 dimensions, the number n of dimensions corresponding to the rows of the operator can be specified:S = SparseOperator(A, n)If not specified, n=1 is assumed.A sparse operator can be converted to a regular array, to a regular matrix or to a sparse matrix.  Assuming S is a SparseOperator, convertions to other representations are done by:A = Array(S)     # convert S to an array\nM = Matrix(S)    # convert S to a matrix\n\nusing SparseArrays\nsp = sparse(S)   # convert S to a sparse matrixnote: Note\nIf the sparse operator S has multi-dimensional columns/rows, these dimensions are preserved when S is converted to an array but are silently flattened when S is converted to a matrix or to a sparse matrix.Package [LinearInterpolators][LinearInterpolators] provides a SparseInterpolator which is a LazyAlgebra LinearMapping and which can also be converted to a SparseOperator (sse the documentation of this package)."
},
