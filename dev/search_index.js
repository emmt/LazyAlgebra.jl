...

{
    ...
    "category": "section",
    "text": "An important feature of LazyAlgebra framework for mappings is that a number of simplifications are automatically made at contruction time.  For instance, assuming A is a mapping:B = A\'\nC = B\'yields C which is just a reference to A. In other words, that is adjoint(adjoint(A)) -> A holds.  LikelyD = inv(A)\nE = inv(D)yields E which is another reference to A.  In other words, inv(inv(A)) -> A holds assuming by default that A is invertible.  This follows the principles of laziness.  It is however, possible to prevent this by extended the Base.inv method so as to throw an exception when applied to the specific type of A:Base.inv(::SomeNonInvertibleMapping) = error(\"non-invertible mapping\")where SomeNonInvertibleMapping <: Mapping is the type of A.Other example of simplifications:B = 3A\nC = 7B\'where mappings B and C are such that B*x ≡ 3*(A*x) and C*x ≡ 21*(A*x) for any vector x.  That is C*x is evaluated as 21*(A*x) not as 7*(3*(A*x)) thanks to simplifications occurring at the contruction of the mapping C.Using the ≡ to denote in the right-hand side the actual construction made by LazyAlgebra for the expression in the left-hand side and assuming A, B and C are linear mappings, the following simplications will occur:(A + C + B + 3C)\' ≡ A\' + B\' + 4C\'\n(A*B*3C)\'         ≡ 3C\'*B\'*A\'\ninv(A*B*3C)       ≡ 3\\inv(C)*inv(B)*inv(A)However, if M is a non-linear mapping, then:inv(A*B*3M) ≡ inv(M)*(3\\inv(B))*inv(A)which can be compared to inv(A*B*3C) when all operands are linear mappings.Note that, due to the associative rules applied by Julia, parentheses are needed around constructions like 3*C if it has to be interpreted as 3C in all contexes.  Otherwise, A*B*(3*C) is equivalent to A*B*3C while A*B*3*C is interpreted as ((A*B)*3)*C; that is, compose A and B, apply A*B to 3 and right multiply the result by C."
},
