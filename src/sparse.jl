...
A simple (but slow for CSR and CSC storage formats) method to loop over the coordinates
and values of the structural non-zero of a sparse operator `A` is to write:
