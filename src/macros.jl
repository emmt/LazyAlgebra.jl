...
`T`. The `LazyAlgebra.@callable` macro can also prefix the definition of the structure `T`.

!!! note
    Since Julia 1.3, methods can be added to an abstract type and it is not necessary to use
    this macro for user-defined operators unless retro-compatibility is needed.
...
function try_get_struct_name_from_definition(ex::Expr)
    # In a structure definition, args[1] = true if mutable, false otherwise, args[2] is the
    # structure name + type parameters (if any) + parent type (if any).
